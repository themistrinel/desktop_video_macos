...
- (void)applicationDidFinishLaunching:(NSNotification*)aNotification
{
    ...
    NSString* lastPath = [[NSUserDefaults standardUserDefaults] stringForKey:kLastVideoPathKey];
...
- (void)setupMenuBar
{
    ...
    [menu addItemWithTitle:NSLocalizedString(@"Change Video...", @"Menu item to change video")
    ...
    NSMenuItem* muteItem =
        [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"Mute", @"Menu item to mute video")
                                   action:@selector(toggleMute:)
                            keyEquivalent:@"m"];
    muteItem.state = [[NSUserDefaults standardUserDefaults] boolForKey:kIsMutedKey]
                         ? NSControlStateValueOn
                         : NSControlStateValueOff;
    [menu addItem:muteItem];

    NSMenuItem* loginItem = [[NSMenuItem alloc]
        initWithTitle:NSLocalizedString(@"Start at Login", @"Menu item to toggle start at login")
               action:@selector(toggleLoginItem:)
        keyEquivalent:@""];
    BOOL shouldStartAtLogin = [[NSUserDefaults standardUserDefaults] boolForKey:kStartAtLoginKey];
    loginItem.state = shouldStartAtLogin ? NSControlStateValueOn : NSControlStateValueOff;
...
{
    [menu removeAllItems];
    NSArray* recents = [[NSUserDefaults standardUserDefaults] stringArrayForKey:kRecentVideosKey];
    if (!recents || recents.count == 0)
    {
        NSMenuItem* none = [[NSMenuItem alloc] initWithTitle:NSLocalizedString(@"None", @"")
                                                      action:nil
                                               keyEquivalent:@""];
        [none setEnabled:NO];
        [menu addItem:none];
...
- (void)removeRecentVideoPath:(NSString*)path
{
    NSMutableArray* recents =
        [[[NSUserDefaults standardUserDefaults] stringArrayForKey:kRecentVideosKey] mutableCopy];
    if (recents)
    {
        [recents removeObject:path];
        [[NSUserDefaults standardUserDefaults] setObject:recents forKey:kRecentVideosKey];
...
- (void)changeVideo:(id)sender
{
    ...
    panel.title = NSLocalizedString(@"Select a video", @"Title for the open panel");
...
- (void)loadVideo:(NSURL*)videoURL
{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:videoURL.path forKey:kLastVideoPathKey];
    ...
    [asset loadValuesAsynchronouslyForKeys:keys
                         completionHandler:^{
                             dispatch_async(dispatch_get_main_queue(), ^{
                                 NSError* error = nil;
                                 AVKeyValueStatus status = [asset statusOfValueForKey:@"playable"
                                                                                error:&error];

                                 if (status == AVKeyValueStatusLoaded && asset.playable)
                                 {
                                     [self finalizePlayerSetupWithAsset:asset videoURL:videoURL];
                                 }
                                 else
                                 {
                                     ...
                                     alert.messageText = NSLocalizedString(@"Video Error", @"");
...
- (void)finalizePlayerSetupWithAsset:(AVAsset*)asset videoURL:(NSURL*)videoURL
{
    ...
    self.playerLooper = [AVPlayerLooper playerLooperWithPlayer:self.player templateItem:playerItem];
    self.player.muted = [[NSUserDefaults standardUserDefaults] boolForKey:kIsMutedKey];
    [self.player play];
    self.isVisible = YES;
    [self setupWindows];
    NSLog(@"[DesktopVideo DevLog] Reprodução confirmada: %@", videoURL.lastPathComponent);
}
...
- (void)addLegacyLoginItem
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSString* bundleName =
            [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleName"] ?: @"MyDesktopVideo";
        NSString* source =
...
- (void)removeLegacyLoginItem
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSString* bundleName =
            [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleName"] ?: @"MyDesktopVideo";
        NSString* source =
            [NSString stringWithFormat:@"tell application \"System Events\" to delete "
...
{
    BOOL shouldBeEnabled = [[NSUserDefaults standardUserDefaults] boolForKey:kStartAtLoginKey];
    ...
    if (@available(macOS 13.0, *))
    {
        ...
        if (shouldBeEnabled)
        {
            if (service.status != SMAppServiceStatusEnabled)
            {
                ...
                {
                    NSLog(@"[LoginItem] SMAppService falhou: %@", error.localizedDescription);
                }
            }
...
- (void)toggleLoginItem:(NSMenuItem*)sender
{
    ...
    [[NSUserDefaults standardUserDefaults] setBool:enable forKey:kStartAtLoginKey];
    [[NSUserDefaults standardUserDefaults] synchronize];
    ...
    if (@available(macOS 13.0, *))
    {
        ...
        if (enable)
        {
            ...
            if (![service registerAndReturnError:&error])
            {
                NSLog(@"[LoginItem] SMAppService falhou no toggle: %@", error.localizedDescription);
            }
            [self addLegacyLoginItem];
        }
        else
